function DP_no_deg(                                                                                                   # Ora conosco per ogni settimana i valori di inflow e prezzo (calcolati con il modello di Markov) - risolvo il problema come "DETERMINISTICO"
  ...
  @unpack (grid_Capacity, min_SOC, energy_Capacity, power_Capacity, Eff_charge, Eff_discharge, NFull) = Battery;      
  ...
  @timeit to "Solve dynamic programming" begin
      ...
      for iState=1:NStates                                                      # Considero gg=365gg*24h*2 tutti i possibili stati
        ...
        soc_start = seg[iState]
          ...
          for jState=1:NStates                                                  # Considero tutti gli stati allo stage successivo
            ...
            val[t,iState,jState] = Power_prices[t]*NHoursStep*(BESS_discharge[t,iState,jState]+PV_grid[t,iState,jState]) + incentivo[t]*shared_energy[t,iState,jState]*NHoursStep -penalty_Export -penalty_SOC - penalty_PV + optimalValueStates[t+1,jState]      #/10E5
    ...
    let startingFrom =a, comingFrom=0
      for t=1:NSteps
        ...
        net_revenues = gain[t,startingFrom,comingFrom] + revenues_shared_energy[t,startingFrom,comingFrom] 
       ...
       # overallCost = overallCost + batCost
        ...
        push!(optimalPath,saveOptimalValues_no_deg(t, optValue, price, startingFrom, comingFrom, seg, charge_bat, dis_bat, pv_grid, pv, net_revenues, sh_en,ld,in))
...
function saveOptimalValues_no_deg(stage::Int64,optimalValue::Float64,price::Float64, curSt::Int64, nextSt::Int64, seg::Any, charge::Float64,discharge::Float64, toGrid::Float64, PV::Float64, netRev::Float64 , sh_en::Float64,ld::Float64,in::Float64)
  optimalStage_no_deg(stage, optimalValue, price, seg[curSt], seg[nextSt], charge, discharge, toGrid, PV,netRev,sh_en,ld,in)
end
